...
def compute_advantages(
    ...
    if use_gae:
        ...
        if last_rset != None:
            vpred_t0 = np.concatenate([rollout['vf_preds0'], np.array([last_rset[0]])])
            vpred_t1 = np.concatenate([rollout['vf_preds1'], np.array([last_rset[1]])])
            rollout['returns0'] = gamma * vpred_t0[1:]
            rollout['returns1'] = gamma * vpred_t1[1:]
...
def compute_gae_for_sample_batch(
    ...
    # Trajectory has been truncated -> last r=VF estimate of last obs.
        ...
        if num_qnet != None:
            last_r0 = policy._value0()
            last_r1 = policy._value1()
