...
class Policy(metaclass=ABCMeta):
    ...
    def _initialize_loss_from_dummy_batch(
        ...
        if 'load_memory' in self.config.keys():
            train_batch['task_idx'] = torch.tensor([0 for _ in range(len(train_batch))]).to(self.device)
            train_batch['from_memory'] = torch.tensor([0 for _ in range(len(train_batch))]).to(self.device)
