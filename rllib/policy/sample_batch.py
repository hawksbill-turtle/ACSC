...
class SampleBatch(dict):
    ...
    def split_by_episode(self, key: Optional[str] = None) -> List["SampleBatch"]:
        ...
        def slice_by_from_memory():
            slices_from_memory = []
            slices_not_from_memory = []
            # Produce slice w/wo memory.
            cur_eps_id = self[SampleBatch.EPS_ID][0]
            offset = 0
            for i in range(self.count):
                fm = self[SampleBatch.FROM_MEMORY][i].item()
                if not fm:
                    slices_not_from_memory.append(self[i:i+1])
                else:
                    slices_from_memory.append(self[i:i+1])
            slices_nfm = self.concat_samples(slices_not_from_memory)
            slices_fm = self.concat_samples(slices_from_memory)
            slices = [slices_nfm, slices_fm]
            return slices

        key_to_method = {
            SampleBatch.EPS_ID: slice_by_eps_id,
            SampleBatch.DONES: slice_by_dones,
            SampleBatch.FROM_MEMORY: slice_by_from_memory
        }
    ...
    for k in concated_samples[0].keys():
        try:
            if k == "infos":
                ...
                if torch.is_tensor(concated_samples[0][k]):
                    data_device = concated_samples[0][k].device
                    concatd_data[k] = torch.tensor(tree.map_structure(
                        _concat_key, *[c[k].to('cpu') for c in concated_samples]
                    ), device=data_device)
                else:
